package com.tsingtec.follow.handler.filter;

import com.tsingtec.follow.utils.XssHttpServletRequestWrapper;
...
public class XssFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        //将不需要xss过滤的接口添加到列表中
        if(StringUtils.hasText(strExcludes)){
            String[] urls = strExcludes.split(",");
            for(String url:urls){
                excludes.add(url);
            }
        }
        if(StringUtils.hasText(strEnabled)){
